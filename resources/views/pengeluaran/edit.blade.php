...
$(document).ready(function() {
    ...
    // Fungsi untuk menghitung total berdasarkan jumlah satuan, nominal, dan lain-lain
    function calculateTotal(key) {
        ...
        $(`#jumlah_${key}`).val(total.toFixed(2)); // Format dua angka desimal
    }

    // Set event listener pada semua input untuk menghitung total
    $(document).on('input', '.jumlah_satuan, .nominal, .dll', function() {
        const key = $(this).data('key'); // Mendapatkan indeks (key) dari elemen saat ini
        calculateTotal(key); // Panggil fungsi untuk menghitung total
    });
});
</script>
